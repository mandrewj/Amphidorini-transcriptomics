...
echo date
echo "Aligning AA fastas in ORTHOMCL sub-directories..."
for GROUP in $(ls -d ORTHOMCL*/)

do
	cd $GROUP
	input="$(echo *_AA.fa)"
	#make sure there is an AA fasta to align, if not chunk the group
	if [ "$input" == "" ]
	then
		cd $startdir
		mv $GROUP failed_groups/no_fasta/
		echo $GROUP had no AA fasta, was removed.
	else
